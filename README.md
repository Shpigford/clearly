...
- **Typography settings** — choose separate editor and preview fonts in Settings, while preview code stays monospaced and Quick Look keeps the bundled default
...
- **Focused tests** — unit coverage checks typography preference fallbacks and preview CSS regression cases
...
Edit `Shared/PreviewCSS.swift`. This CSS is used by both the in-app preview and QuickLook extension. Keep it in sync with `Theme.swift` colors. The app can inject custom preview fonts, but Quick Look stays on the bundled default typography until shared defaults are added in a separate change.
...
Lightweight unit tests cover typography preference fallbacks, reset behavior, and preview code monospace rendering. Validate the app manually too:

1. Build and run the app (Cmd+R)
2. Open Settings and verify editor font choices stay monospaced
3. Switch to preview mode (⌘2) and verify rendered output
4. Export or print a document and verify the preview font carries through
5. Test QuickLook by selecting a `.md` file in Finder and pressing Space
