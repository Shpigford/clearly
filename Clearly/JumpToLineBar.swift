    ...
    var body: some View {
        HStack(spacing: 8) {
            HStack(spacing: 4) {
                ...
                TextField(L10n.string("jumpToLine.placeholder", defaultValue: "Line number"), text: $state.lineText)
                ...
                if state.totalLines > 0 {
                    Text(
                        L10n.format(
                            "jumpToLine.totalFormat",
                            defaultValue: "of %lld",
                            Int64(state.totalLines)
                        )
                    )
            ...
            Button(L10n.string("jumpToLine.go", defaultValue: "Go")) {
                state.commit()
            }
            .buttonStyle(.plain)
