...
struct BacklinksView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 6) {
                Text(L10n.string("backlinks.title", defaultValue: "Backlinks"))
            ...
            if backlinksState.backlinks.isEmpty && backlinksState.unlinkedMentions.isEmpty {
                Text(L10n.string("backlinks.empty", defaultValue: "No other documents link to this file"))
                    .font(.system(size: 12))
                    .foregroundStyle(.tertiary)
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        if !backlinksState.unlinkedMentions.isEmpty {
                            DisclosureGroup {
                                ...
                                Text(
                                    L10n.format(
                                        "backlinks.unlinkedFormat",
                                        defaultValue: "Unlinked (%lld)",
                                        Int64(backlinksState.unlinkedMentions.count)
                                    )
                                )
        ...
        HStack(spacing: 0) {
            ...
            if showLinkButton, let onLink {
                Button(action: onLink) {
                    Text(L10n.string("backlinks.link", defaultValue: "Link"))
                        .font(.system(size: 10, weight: .medium))
                        .foregroundStyle(.secondary)
